...
void spotRightDrive(int speed, int base){

  if (base - speed < 0){
    analogWrite(RightPin2, min(255, -(base - speed)));
    digitalWrite(RightPin1, LOW);

  } else{
    analogWrite(RightPin1, max(0, base - speed));
    digitalWrite(RightPin2, LOW);
  }

  digitalWrite(LeftPin2, LOW);
  analogWrite(LeftPin1, min(255, base + speed));
}

void spotLeftDrive(int speed, int base){
  if (base - speed < 0){
    analogWrite(LeftPin2, min(255, -(base - speed)));
    digitalWrite(LeftPin1, LOW);

  } else{
    analogWrite(LeftPin1, max(0, base - speed));
    digitalWrite(LeftPin2, LOW);
  }

  digitalWrite(RightPin2, LOW);
  analogWrite(RightPin1, min(255, base + speed));
}
...
float getMagX(ICM_20948_I2C *sensor){
  return sensor->magX();
}
...
//////// KF //////////
float drag = 0.0005208;
float momentum = 0.0002827;
float delta_t = 0.0122;
...
Matrix<2,2> sig_u = { 10^2, 0,
                      0, 10^2 };
Matrix<1,1> sig_z = { 1^2 };

Matrix<2,2> I_mat = { 1, 0,
                      0, 1      };
Matrix<2,2> A_d   = { 1, 0.0122,
                      0, 0.9775 };
Matrix<2,1> B_d   = { 0,
                      43.1095   };
...
// Initial states
Matrix<2,2> sig   = { 10, 0,
                      0, 10}; // initial state uncertainty
Matrix<2,1> x_val = { 0, 
                      0      }; // initial state output


int kf(Matrix<2,1> x_val, Matrix<2,2> sig, Matrix<2,1> dist, int intDist, int pwm) {

  Matrix<2,1> x_p = A_d*dist + B_d*pwm;
  Matrix<2,2> sig_p = A_d*sig*(~A_d) + sig_u;

  Matrix<1,1> y_curr = { intDist };
...
void PIDKF(){
  ...
  while (millis() - start_time < timeWindow){
    ...
    // for pure measured distance
    //int error = ONEDistance - targetDistance;

    // for kf distance
    int error = kfDistance - targetDistance;
    ...
    // integrate kf 
    Matrix<2,1>  inputDst = {ONEDistance, 0};
    kfDistance = kf(x_val, sig, inputDst, ONEDistance, speed);
    ...
    // if reached conservative number of logs during sampling(to ensure not running out of memory)
      ...
      digitalWrite(LED_BUILTIN, !digitalRead(LED_BUILTIN));
      sendPIDDebugArr(timeArr, pitchArr, rollArr, yawArr, ONEDistArr, kfArr, speedArr);
...
void step(){
  ...
  while (millis() - start_time < timeWindow){
    ...
    if (ONEDistance < 500 & ONEDistance > 0){
      proximity += 1;
      if (proximity > 10){
        stopDriving();
        speed = 0;
...
void stunt(){
  ...
  while (millis() - start_time < timeWindow){
    ...
    // ONEDistance < targetDistance & kfDistance > 0 & 

    if ((millis() - start_time > 950) & flipped == false){
      proximity += 1;
      if (proximity > 1){
    ...
    if (distanceSensor1.checkForDataReady()){
      ...
      if(logCount == 2){
        ONEDistance = (ONEDistArr[1] - ONEDistArr[0])/(timeArr[1] - timeArr[0])*(millis() - timeArr[1]) + ONEDistArr[1];
      }
    }
...
void angspeed(){
  ...
  while (millis() - start_time < timeWindow){
    ...
    int error = myICM.gyrZ() - targetOffset;

    // positive error -> decreased speed 
    int speed = 100 - (int) (kp_a * error);
    ...
    if (logCount < entries){
      timeArr[logCount] = millis();
      targetArr[logCount] = targetOffset;
      errorArr[logCount] = error;
      yawArr[logCount] = curYawG;
      ONEDistArr[logCount] = ONEDistance;
      rawYawArr[logCount] = myICM.gyrZ();
      speedArr[logCount] = speed;
      logCount += 1;
    } 
    // if reached conservative number of logs during sampling(to ensure not running out of memory)
      ...
      digitalWrite(LED_BUILTIN, !digitalRead(LED_BUILTIN));
      sendPIDDebugArr(timeArr, targetArr, errorArr, yawArr, ONEDistArr, rawYawArr, speedArr);
